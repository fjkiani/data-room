...
const DeckViewerPage: React.FC = () => {
  ...
  const isCustomReact = deck?.slides.some(slide => 
    slide.content.some(content => content.type === 'custom-react')
  ) || false;
  const isCinematic = deck?.slides.some(slide => 
    slide.content.some(content => content.type === 'cinematic' || content.type === 'custom-react')
  ) || false;

  // Derive slide list: for custom-react, treat each content item as a slide surrogate
  const derivedSlides = React.useMemo(() => {
    if (!deck) return [] as { id: string; title: string; content: any[] }[];
    if (!isCustomReact) return deck.slides as any[];
    const first = deck.slides[0];
    return first.content.map((_, idx) => ({ id: `${first.id}-${idx}`, title: first.title || `Slide ${idx + 1}`, content: first.content }));
  }, [deck, isCustomReact]);
  ...
  const handleTouchEnd = () => {
    if (isCustomReact) return; // pass-through for interactive decks
  ...
  const handleScreenClick = (e: React.MouseEvent) => {
    if (isCustomReact) return; // do not hijack click for interactive slides
    ...
    return (
      ...
      >
        ...
        <div className="absolute inset-0 w-full h-full">
          {currentSlide && <SlideRenderer slide={isCustomReact ? deck.slides[0] : currentSlide} />}
          {!isCustomReact && (
          ...
          {!isCustomReact ? (
            ...
            <div className="absolute top-5 right-5 z-50">
              <button onClick={() => navigate('/')} className="px-4 py-2 bg-red-600/70 text-white rounded-xl hover:bg-red-500/80 transition-colors duration-200">Exit</button>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center space-x-4">
              ...
              <div className="border-l border-gray-300 pl-4">
                ...
                {!isCustomReact && (<p className="text-sm text-gray-500">Slide {currentSlideIndex + 1} of {totalSlides}</p>)}
              </div>
            </div>
      ...
      <div className="flex-1 flex h-full">
        ...
        <div className="flex-1 flex flex-col">
          ...
          >
            ...
            {!isCustomReact && (<button onClick={goToPreviousSlide} disabled={currentSlideIndex === 0} className={`px-4 py-2 rounded-lg font-medium transition-colors ${isFullscreen ? 'bg-white bg-opacity-20 text-white hover:bg-opacity-30 disabled:opacity-50' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 disabled:opacity-50'}`}>Previous</button>)}
            <div className="flex items-center space-x-4">
              {!isCustomReact && (<span className={isFullscreen ? 'text-white' : 'text-gray-900'}>{currentSlideIndex + 1} of {totalSlides}</span>)}
            </div>
            {!isCustomReact && (<button onClick={goToNextSlide} disabled={currentSlideIndex === totalSlides - 1} className={`px-4 py-2 rounded-lg font-medium transition-colors ${isFullscreen ? 'bg-white bg-opacity-20 text-white hover:bg-opacity-30 disabled:opacity-50' : 'bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50'}`}>Next</button>)}
          </div>
        </div>
      </div>
    </div>
